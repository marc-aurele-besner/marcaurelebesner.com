...
const PointEvent = ({ color, ...props }: any) => {
...
const CubePoints: React.FC<BufferCubeProps> = ({ route, positionY = 0, ...props }) => {
  const router = useRouter()

  const n = 2_000
  const [positionA] = React.useState(() => makeBuffer({ length: n * 3 }, () => MathUtils.randFloatSpread(1)))
  const [positionB] = React.useState(() => makeBuffer({ length: n * 3 }, () => MathUtils.randFloatSpread(2)))
